...
export default function CustomVisual(props: Props) {
			...
			for (let k = 0; k < cubeWidth; k += 1) {
				...
				let tag = (
					<span
						...
						className={classNames('table h-full w-full rounded', empty && '!opacity-0')}
			...
			for (let i = 0; i < cubeSize; i += 1) {
				for (let k = 0; k < cubeSize; k += 1) {
					...
					let tag = (
						<span
							key={`cubeNumber-${off}`}
							className="table h-full w-full rounded"
							style={{backgroundColor: color || DEFAULT_COLOR}}
						/>
					);
	...
	let body;
	if (threeD) {
		body = <div className="relative z-[100] h-px w-px">{get3DCubelets()}</div>;
	} else {
		body = (
			<div
	...
	return (
		<div
			style={style}
			className={classNames(threeD && 'flex items-center justify-center pt-[50px]')}
		>
			{body}
		</div>
	);
