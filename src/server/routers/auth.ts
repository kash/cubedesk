...
import {getUserByEmail, getUserByUsername, sanitizeUser, createUserAccount, updateUserAccount, updateUserAccountPassword} from '@/server/models/user_account';
...
const SignupInputSchema = z.object({
	...
	email: z.string().email(),
	username: z.string().min(2).max(18).regex(/^[a-zA-Z0-9_]+$/, {
...
export const authRouter = createTRPCRouter({
	login: publicProcedure
		...
		.mutation(async ({input, ctx}) => {
			...
			ctx.resHeaders.set('Set-Cookie', `session=${jwt}; Max-Age=2147483647; HttpOnly; Path=/`);
	...
	signup: publicProcedure
		...
		.mutation(async ({input, ctx}) => {
			...
			let ip = ctx.req.headers.get('x-forwarded-for') || ctx.req.headers.get('x-real-ip') || 'unknown';
			if (Array.isArray(ip)) {
				ip = ip[0];
			...
			// Create user account
			const user = await createUserAccount(firstName, lastName, email, username, password, ip);
	...
	updateUserAccount: userProcedure
		...
		.mutation(async ({input, ctx}) => {
			...
			const updatedUser = await updateUserAccount(me.id, firstName, lastName, email, username);
