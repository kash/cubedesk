...
import {friendshipRequestRouter} from '@/server/routers/friendship/request';
import {Friendship, FriendshipRequestSchema, FriendshipSchema} from '@/generated/zod';
import {
	getPaginatedResponse,
	getPaginationInputSchema,
	getPaginationOutputSchema,
...
export const friendshipRouter = createTRPCRouter({
	...
	request: friendshipRequestRouter,
	list: userProcedure
		.input(getPaginationInputSchema())
		.output(getPaginationOutputSchema(FriendshipSchema))
		.query(async ({ctx, input}) => {
			const parsedInput = parsePaginationInput(input);

			return getPaginatedResponse<Friendship, 'friendship'>('friendship', parsedInput, {
				where: {
					user_id: ctx.me.id,
				},
			});
	...
	friendships: userProcedure.input(paginationArgsInput).query(async ({ctx, input}) => {
	...
	sentFriendshipRequestsToUser: userProcedure.input(userIdInput).query(async ({ctx, input}) => {
		const prisma = getPrismaClient();
		const targetUser = await getUserByIdOrThrow404(input.userId);
