...
export default function ManageUser(props: Props) {
  ...
  const { data: userData, isLoading, refetch } = api.admin.getUserAccountForAdmin.useQuery({
    userId,
  });
  ...
  function getGenericTable(title: string, obj: any) {
  ...
  function getGenericGrid(title: string, list: any[], name: string, sub?: string) {
    let output = list || [];
